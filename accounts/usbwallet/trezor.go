...
import (
	...
	"github.com/justchain/go-justchain/accounts/usbwallet/internal/trezor"
...
// or whtster the justchain app was not started on it.
...
// justchain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.justchainAddress)
	if _, err := w.trezorExchange(&trezor.justchainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.justchainSignTx{
	...
	response := new(trezor.justchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.justchainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
